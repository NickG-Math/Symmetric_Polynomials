...
namespace Symmetric_Polynomials
{
	...
	struct Half_Idempotent_Variables : public Array_Vector_Wrapper<T, N>
	{
		...
		Half_Idempotent_Variables operator+(const Half_Idempotent_Variables &b) const;
	...
	///	@note			This class does NOT provide functions for degrees or variable names: these are provided as pointers directly in Half_Idempotent_Basis
	///	@tparam T 		The (integral) value type of the exponent vector.
	///	@tparam _deg	The (integral) value type used in the degree function.
	...
	public:
		...
		friend class Polynomial_Basis<Half_Idempotent_Basis<_xy_container, _chern_container>, _xy_container, _chern_container>; ///<Befriending parent for CRTP.
	...
	///	@brief				Aliases Half_Idempotent_Basis for certain default template parameters
	...
	using Half_Idempotent_Basis_Default = Half_Idempotent_Basis<default_container<scl_t, Half_Idempotent_Variables<exp_val_t, deg_t>, ordered>, default_container<scl_t, Twisted_Chern_Variables<exp_val_t, deg_t>, ordered>>;
