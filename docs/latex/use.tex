...
For a quick demonstration you may compile the \mbox{\hyperlink{Demo_8cpp}{Demo.\+cpp}}. Eg (on Linux) \begin{DoxyVerb}clang++ source/Demo.cpp -std=c++17 -O3 -march=native -o Executable.out      
\end{DoxyVerb}


You can then run the file Executable.\+out. You can similarly do this over Windows using M\+S\+VC, producing a .exe file . Make sure to use the {\ttfamily -\/std=c++17} flag (or set the language options in an I\+DE to support C++17).\hypertarget{use_exa}{}\doxysection{Code examples}\label{use_exa}
...
A polynomial is made out of a vector of monomials eg \begin{DoxyVerb}polynomial<int, halfidempotent>({monomial<int,halfidempotent>(2,{1,2,0,1}), monomial<int,halfidempotent>(3,{3,0,1,1})});
\end{DoxyVerb}
...
When we have no relations, we can use \begin{DoxyVerb}polynomial<rational, norelations> poly({ monomial<rational, norelations>(2, { 1,2 }), monomial<rational, norelations>(2, {2,1 }) }));
...
When we have the \char`\"{}half idempotent relation\char`\"{}, the generators are $\alpha, c_i, \beta_{s,t}$ (see \mbox{\hyperlink{math}{The Math}}). In the program, {\ttfamily Idem} is the dominant term of $\alpha$, {\ttfamily Chern} is a vector containing the dominant terms of $c_1,...,c_n$ and {\ttfamily Twisted\+Chern} is a vector containing the dominant terms of $\beta_{1,1},...,\beta_{1,n-1},\beta_{2,1},...,\beta_{n-1,1}$ in this order. To get the original indexing on the $\beta$ elements we use {\ttfamily std\+::map$<$std\+::pair$<$int,int$>$,int$>$ Twisted\+Chern\+\_\+indexer} eg \begin{DoxyVerb}std::cout << TwistedChern_indexer[std::make_pair<int,int>(1,2)];
\end{DoxyVerb}


prints {\ttfamily 1} since {\ttfamily Twisted\+Chern\mbox{[}1\mbox{]}} corresponds to $\beta_{1,2}$.

The code \begin{DoxyVerb}polynomial<rational, halfidempotent> poly({ monomial<rational, halfidempotent>(2, { 1,0,1,0 }), monomial<rational, halfidempotent>(2, {0,1,0,1 }) })
